...
def _import_sample_cases_module(module_name: str = "sample_cases"):
...
def _import_orchestrator_module(orchestrator_module: str = "demo_orchestrator"):
    ...
    if module_ref.endswith(".py") or any(sep in module_ref for sep in ("/", "\\")):
        ...
        module_name = f"{module_path.stem}_regression_runtime"
...
def validate_result(case: CaseDict, result: ResultDict, orchestrator: MinimalOrchestrator) -> Dict[str, Any]:
    ...
    next_topic_state = _safe_get(result, "final_plan", "next_topic_state", default={}) or {}
    planner_meta = _safe_get(result, "final_plan", "planner_meta", default={}) or {}
    ...
    main_speakers = [item.get("agent_id") for item in role_assignment if item.get("role") == ROLE_MAIN]
    supplement_speakers = [item.get("agent_id") for item in role_assignment if item.get("role") == ROLE_SUPPLEMENT]

    preferred_main_any = expected.get("preferred_main_any", []) or []
    if preferred_main_any:
        checks.append(CheckItem(
            ok=any(agent_id in preferred_main_any for agent_id in main_speakers),
            name="preferred_main_any",
            actual=main_speakers,
    ...
    if preferred_supplement_any:
        checks.append(CheckItem(
            ok=any(agent_id in preferred_supplement_any for agent_id in supplement_speakers),
            name="preferred_supplement_any",
            actual=supplement_speakers,
    ...
    if forbid_main:
        checks.append(CheckItem(
            ok=all(agent_id not in forbid_main for agent_id in main_speakers),
            name="forbid_main",
            actual=main_speakers,
...
def run_regression_suite(
    ...
    sample_cases_mod = _import_sample_cases_module()
